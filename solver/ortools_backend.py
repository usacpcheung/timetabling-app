...
This module contains the OR-Tools CP-SAT implementation that powers the timetable
solver. The ``build_model`` function creates all variables and constraints for
the scheduling problem, and :func:`solve_cp_sat_model` executes the solver and
extracts the results. Each decision variable is a boolean indicating whether a
particular lesson occurs. Keeping this optimization code separate from the Flask
...
def solve_cp_sat_model(
    ...
    *,
    ...
    Returns:
        :class:`SolverResult` describing the solver status, chosen assignments,
        unsatisfied assumptions (when infeasible) and progress messages.
    """
    ...
    status = _STATUS_MAP.get(raw_status, SolverStatus.UNKNOWN)
            ...
            if solver.BooleanValue(var):
                ...
                for (s, t, sub, sl, l), lv in loc_vars.items():
                    if s == sid and t == tid and sub == subj and sl == slot and solver.BooleanValue(lv):
                        loc = l
                        break
                assignments.append(Assignment(sid, tid, subj, slot, loc))

    core: List[AssumptionInfo] = []
    if (raw_status == cp_model.INFEASIBLE and assumption_registry and
